...
//		
        ...
        for (int i = 0; i <4; i++) {
        	Players[i] = new Player(MainMenu.getPlayerColor(i));
        	System.out.println(MainMenu.getPlayerColor(i).toString());
	   ...
	   currentPlayer.setVisible(true);
		...
		leftPanel_Game.add(lblPlayer4Score);
         ...
         btnSave.setBounds(20, 250, 100, 100);
	...
	public void mouseEntered(MouseEvent e) {
		...
		for (int i = 0; i < ArrayOfPieceButtons.length; i++) {
			...
			for (int j = 0; j <ArrayOfPieceButtons[0].length; j++) {
				...
				if( ArrayOfPieceButtons[i][j].isVisible() & !( (boardBtnrowPos+rowDist) <0 || boardBtnrowPos+rowDist>= btnBoard.length || boardBtncolPos+colDist<0	|| boardBtncolPos+colDist>=btnBoard.length ) ) {
		...
		for (int i = 0; i < ArrayOfPieceButtons.length; i++) {
			...
			for (int j = 0; j <ArrayOfPieceButtons[0].length; j++) {
				...
				if( ArrayOfPieceButtons[i][j].isVisible() & !( (boardBtnrowPos+rowDist) <0 || boardBtnrowPos+rowDist>= btnBoard.length || boardBtncolPos+colDist<0	|| boardBtncolPos+colDist>=btnBoard.length ) ) {
					...
					btnBoard[boardBtnrowPos+rowDist][boardBtncolPos+colDist].setBackground(btnBoardTemp[boardBtnrowPos+rowDist][boardBtncolPos+colDist].getBackground());
			...
			for (int i = 0; i < ArrayOfPieceButtons.length; i++) {
				...
				for (int j = 0; j <ArrayOfPieceButtons[0].length; j++) {
					...
					if ( ArrayOfPieceButtons[i][j].isVisible()  ) {
						...
						else if (btnBoardTemp[boardBtnrowPos+rowDist][boardBtncolPos+colDist].getBackground() != Color.WHITE) {
							throw new Exception("Pieces overlap!");
							}
						
						//checks for correct corner colors for valid placement
						 if ((boardBtnrowPos+rowDist+1 < row && boardBtncolPos+colDist+1 < col && btnBoardTemp[boardBtnrowPos+rowDist+1][boardBtncolPos+colDist+1].getBackground() == ArrayOfPieceButtons[i][j].getBackground() || 
								 boardBtnrowPos+rowDist-1 >= 0 && boardBtncolPos+colDist+1 < col && btnBoardTemp[boardBtnrowPos+rowDist-1][boardBtncolPos+colDist+1].getBackground() == ArrayOfPieceButtons[i][j].getBackground() ||
								 boardBtnrowPos+rowDist+1 < row && boardBtncolPos+colDist-1 >= 0 && btnBoardTemp[boardBtnrowPos+rowDist+1][boardBtncolPos+colDist-1].getBackground() == ArrayOfPieceButtons[i][j].getBackground() ||
								 boardBtnrowPos+rowDist-1 >= 0 && boardBtncolPos+colDist-1 >= 0 && btnBoardTemp[boardBtnrowPos+rowDist-1][boardBtncolPos+colDist-1].getBackground() == ArrayOfPieceButtons[i][j].getBackground())
								)
						{ 
							cornerTouching = true;
						}
						...
						//checks if piece is in starting corners
						 if ((boardBtnrowPos+rowDist == 0 || boardBtnrowPos+rowDist == row-1) && (boardBtncolPos+colDist == 0 || boardBtncolPos+colDist == col-1) ) 
						{
							cornerTouching = true;
						}
						 
						 //checks for color not being beside itself
						 if ((boardBtnrowPos+rowDist+1 < row && btnBoardTemp[boardBtnrowPos+rowDist+1][boardBtncolPos+colDist].getBackground() == ArrayOfPieceButtons[i][j].getBackground() || 
								 boardBtnrowPos+rowDist-1 >= 0  && btnBoardTemp[boardBtnrowPos+rowDist-1][boardBtncolPos+colDist].getBackground() == ArrayOfPieceButtons[i][j].getBackground() ||
								 boardBtncolPos+colDist+1 < col && btnBoardTemp[boardBtnrowPos+rowDist][boardBtncolPos+colDist+1].getBackground() == ArrayOfPieceButtons[i][j].getBackground() ||
								 boardBtncolPos+colDist-1 >= 0 && btnBoardTemp[boardBtnrowPos+rowDist][boardBtncolPos+colDist-1].getBackground() == ArrayOfPieceButtons[i][j].getBackground())
								)
						{ 
							sideTouching = true;
						}	
	...
	public void saveBoard() {
        ...
        try (FileWriter writer = new FileWriter("save.txt");
             BufferedWriter gamesave = new BufferedWriter(writer)) {
...
//            		gamesave.write(fullString + '\n');
            		if (btnBoard[i][j].getBackground() == MainMenu.getPlayerColor(0)) {
            			gamesave.write("1" + '\n');
            		}
            		if (btnBoard[i][j].getBackground() == MainMenu.getPlayerColor(1)) {
            			gamesave.write("2"+ '\n');
            		}
            		if (btnBoard[i][j].getBackground() == MainMenu.getPlayerColor(2)) {
            			gamesave.write("3"+ '\n');
            		}
            		if (btnBoard[i][j].getBackground() == MainMenu.getPlayerColor(3)) {
            			gamesave.write("4"+ '\n');
	...
	public void savePlayer() {
        ...
        try (FileWriter writer = new FileWriter("save1.txt");
             BufferedWriter gamesave = new BufferedWriter(writer)) {
            while (cp != 4) {
            		for (int i = 0; i < 21; i++) {
	...
	public void savePlayerIndex() {
        ...
        try (FileWriter writer = new FileWriter("save2.txt");
             BufferedWriter gamesave = new BufferedWriter(writer)) {
        	 gamesave.write(str1 + "");
...
//					}
					...
					if (pr ==1) {
						btnBoard[j][i].setBackground(MainMenu.getPlayerColor(0));
					}
					else if (pr==2) {
						btnBoard[j][i].setBackground(MainMenu.getPlayerColor(1));
					}	
					else if (pr==3) {
						btnBoard[j][i].setBackground(MainMenu.getPlayerColor(2));
					}
					else if (pr==4) {
						btnBoard[j][i].setBackground(MainMenu.getPlayerColor(3));
					}
					else if (pr==0) {
						btnBoard[j][i].setBackground(Color.WHITE);
					}
